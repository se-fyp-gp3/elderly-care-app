...
const MESSAGES: Record<VoiceLanguage, Record<string, string>> = {
  ...
  en: {
    ...
    medication_all_taken:
      "You've taken all your medication for today! Well done!",
    medication_none_pending: "You have no pending medication right now.",
    medication_error:
      "There was a problem recording your medication. Please try again later.",
    check_no_medication: "You have no medication to take today.",
    add_medication_prompt:
      "OK, please tell me the medicine name and when to take it.",
    ...
    schedule_error:
      "There was a problem setting the schedule. Please try again later.",
    schedule_missing_info:
      "Please tell me when and what you'd like to schedule.",
...
function parseAbsoluteDateCandidate(value: string): Date | null {
  ...
  if (
    !/^\d{4}-\d{1,2}-\d{1,2}|^\d{4}\/\d{1,2}\/\d{1,2}|T\d{2}:\d{2}|Z$|[+-]\d{2}:?\d{2}$/.test(
      normalized,
    )
  ) {
    return null;
...
function getExplicitDayOffset(text: string): number | null {
  ...
  if (/(聽朝|听朝|聽日|听日|明天|明日|明早|明朝|tomorrow)/i.test(text))
    return 1;
  if (
    /(今日|今天|今朝|今晚|today|tonight|this morning|this afternoon|this evening)/i.test(
      text,
    )
  ) {
    return 0;
...
function applyTimeOfDayHint(hours: number, text: string): number {
  const hasPmHint =
    /(下午|下晝|晚上|今晚|夜晚|傍晚|pm|afternoon|evening|tonight)/i.test(text);
...
function parseSpokenScheduleDate(
  text: string,
...
function extractScheduleTitle(text: string): string {
  ...
  title = title.replace(
    /\b(confirm|confirmed|ok|okay|sure|please|set|schedule|appointment|tomorrow|today|tonight|this morning|this afternoon|this evening|am|pm)\b/gi,
    " ",
  );
  title = title.replace(
    /(確認|确认|確定|确定|請|请|幫我|帮我|安排|新增|添加|設定|设定|日程|行程|預約|预约|時間|时间|喺|在|於|于|聽朝|听朝|聽日|听日|明天|明日|明早|明朝|今日|今天|今朝|今晚|後天|后天|朝早|早上|上午|下晝|下午|晚上|夜晚|中午|凌晨)/g,
    " ",
  );
  title = title.replace(
    /(\d{1,2})\s*(?:點鐘|点钟|點半|点半|點|点|時|时|:)\s*(\d{1,2})?\s*(?:分)?/g,
    " ",
  );
...
function resolveScheduleDraft(
  ...
  const explicitTitle = String(
    params.title || params.event || fallback?.title || "",
  ).trim();
  const explicitDescription = String(
    params.description || fallback?.description || "",
  ).trim();
  ...
  const title =
    explicitTitle ||
    candidateTexts.map((text) => extractScheduleTitle(text)).find(Boolean) ||
    "";
...
const ADD_MED_MESSAGES: Record<VoiceLanguage, Record<string, string>> = {
  yue: {
    ...
    invalid_times_per_day:
      "「每日幾次」要係一個整數，例如一日三次。請再講一次。",
  ...
  en: {
    missing_name:
      "What medicine would you like to add? Please tell me the name.",
    missing_times: "How many times a day?",
    missing_duration: "How many days should you take it?",
    invalid_times_per_day:
      "Times per day must be a whole number, like 3 times a day. Please say again.",
    invalid_duration:
      "Duration must be a whole number of days, like 7 or 14 days. Please say again.",
...
async function handleAddMedication(
  ...
  const drugName =
    params.drug_name || params.name || params.medication_name || "";
  ...
  if (times.length === 0) {
    ...
    times =
      defaultTimes[timesInt] ||
      Array.from({ length: timesInt }, (_, i) => {
...
export async function executePendingAction(
  ...
  userId: string,
  language: VoiceLanguage = "yue",
  latestRecognition?: Pick<
    VoiceRecognitionResult,
    "params" | "transcript" | "reply"
  >,
): Promise<CommandResult> {
  const msg = MESSAGES[language] || MESSAGES.yue;
  const addMsg = ADD_MED_MESSAGES[language] || ADD_MED_MESSAGES.yue;
  ...
  try {
    switch (pendingData.action) {
      case "add_medication": {
        const times: string[] = Array.isArray(pendingData.reminder_times)
          ? pendingData.reminder_times
          : [];
      ...
      case "set_schedule": {
        ...
        const datetime =
          draft.datetime ||
          parseAbsoluteDateCandidate(
            String(pendingData.datetimeIso || pendingData.datetime || ""),
          );
...
export async function synthesizeCommandResponse(
  ...
  try {
    // Guard: skip TTS for empty or JSON-shaped messages so we never crash the
    // synthesis function with garbage. (Recognition occasionally produces
    // malformed responses with smart quotes that bypass our parser.)
    ...
    if (looksLikeJson) {
      console.warn(
        "[voice-cmd] Skipping TTS — message looks like JSON garbage:",
        trimmed.slice(0, 80),
      );
      return null;
    }
    // Cap message length to avoid pushing the function past its sync timeout.
    const safeMessage = trimmed.length > 500 ? trimmed.slice(0, 500) : trimmed;
    ...
    const result = await synthesizePersonalVoice(
      safeMessage,
      voiceId,
      undefined,
      language,
