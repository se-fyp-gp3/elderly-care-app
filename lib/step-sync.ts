 ...
 *
 * Handles:
 * 1. Google Fit via Health Connect (Android) / Apple HealthKit (iOS)
 *    step count authorization & data retrieval
...
export type StepDataSource = "health_connect" | "apple_healthkit" | "manual";
 ...
 * - Android: Health Connect (reads Google Fit data)
 * - iOS: Apple HealthKit
 *
...
async function requestHealthConnectAuthorization(): Promise<boolean> {
  try {
    ...
    if (!hasPermission) {
      console.warn("[HealthConnect] Activity recognition permission denied");
      return false;
    }
