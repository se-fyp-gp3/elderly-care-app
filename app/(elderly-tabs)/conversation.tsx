...
export default function ElderlyConversation() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        if (elderly) {
          setMyProfileId(elderly.$id);
          setMyName(elderly.name || user.name || "Me");
        }
      } catch (error) {
        console.error("Error loading elderly profile:", error);
      } finally {
        setLoading(false);
      }
    })();
  ...
  return (
    <ConversationScreen
      ...
      contactRole={(params.contactRole as "caregiver" | "elderly") || "caregiver"}
    />
  );
}
