...
export default function MedicationManagement() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const mappedMeds: MedicationItem[] = response.rows.map((row) => {
        ...
        const timeString = (row.approx_times || [])
          .map((t) => {
            try {
        ...
        return {
          ...
          lastTaken: row.last_taken
            ? new Date(row.last_taken).toLocaleString([], {
                month: "short",
                day: "numeric",
                hour: "2-digit",
                minute: "2-digit",
  ...
  const pendingCount = medicationsState.filter(
    (m) => m.status === "pending",
  ).length;
  const completedCount = medicationsState.filter(
    (m) => m.status === "completed",
  ).length;
  const overdueCount = medicationsState.filter(
    (m) => m.status === "overdue",
  ).length;
    ...
    >
      ...
      <Portal>
        ...
        >
          ...
          <Dialog.Content>
            <Text>
              Please make sure the elderly have taken their medication on time.
            </Text>
            <TextInput
          ...
          <Dialog.Actions>
            ...
            <Button
              onPress={() => {
                if (dialogVisible !== null) onConfirmTaking(dialogVisible);
              }}
            >
              Confirm completion
            </Button>
